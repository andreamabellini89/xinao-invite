...
export default function GuestInvitePage() {
  ...
  // ── Load event + guest from Supabase ──
  useEffect(() => {
    const load = async () => {
      setLoading(true)
      try {
        ...
        if (gst.status === 'registered' || gst.status === 'checked-in') {
          ...
          setForm(f => ({
            ...f,
            firstName: gst.first_name || '',
            lastName:  gst.last_name  || '',
            email:     gst.email      || '',
            phone:     gst.phone      || '',
          }))
          setStep('form')
  ...
  // ── Submit registration ──
  const submit = async () => {
    if (!validate() || !event || !guest) return
    setSubmitting(true)
    setSubmitError(null)
  ...
  if (step === 'already' && saved && event) return (
    <div style={{ minHeight:'100vh', background:T.cream }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'24px 16px' }}>
        ...
        <div style={{ fontSize:9, letterSpacing:'0.3em', color:T.gold, fontFamily:'sans-serif', marginBottom:14 }}>YOUR INVITATION</div>
  ...
  if (step === 'success' && saved && event) return (
    <div style={{ minHeight:'100vh', background:T.cream }}>
      <TopBar section="GUEST PORTAL" />
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'24px 16px' }}>
        <div style={{ width:'100%', maxWidth:440, background:T.greenBg, border:`1px solid ${T.greenBorder}`, borderRadius:4, padding:'14px 18px', marginBottom:20, display:'flex', alignItems:'center', gap:10 }}>
        ...
        <div style={{ width:'100%', maxWidth:420 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ textAlign:'center' }}>
              <a href="/" style={{ color:T.n400, fontFamily:'sans-serif', fontSize:10, letterSpacing:'0.15em', textDecoration:'none' }}>
                ← BACK TO MENU
              </a>
            </div>
          </div>
