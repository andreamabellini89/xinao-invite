...
import type { XinaoEvent, EventFormData, Guest } from '@/types'
...
// ── Helpers ───────────────────────────────────────────────────────────────────
async function uploadImage(file: string, eventId: string): Promise<string | null> {
  // file is a base64 dataURL — convert to Blob
...
function EventFormModal({
  ...
  const [form, setForm] = useState<EventFormData>(
    initial
      ? { name:initial.name, subtitle:initial.subtitle??'', date:initial.date, time:initial.time, location:initial.location, address:initial.address??'', description:initial.description??'', status:initial.status, cover_image_url:initial.cover_image_url }
      : blank
  )
      ...
      <div style={{ background:T.white, borderRadius:4, padding:'28px 24px', maxWidth:520, width:'100%', border:`1px solid ${T.n200}`, maxHeight:'90vh', overflowY:'auto' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          {([
            ...
            ['Date *','date','24 JUNE 2026'],
            ['Time *','time','8:00 PM'],
            ['Venue / Location *','location'],
            ['Full Address','address'],
            ['Description / Tagline','description'],
          ] as [string, keyof EventFormData, string?][]).map(([label, key, ph]) => (
            <div key={key}>
              ...
              <input
                ...
                style={{ width:'100%', padding:'10px 13px', border:`1px solid ${T.n200}`, borderRadius:2, fontSize:13, fontFamily:'sans-serif', color:T.n800, outline:'none', background:T.white, boxSizing:'border-box' }}
              />
            </div>
          ))}
          <div>
            ...
            <select value={form.status} onChange={e => set('status', e.target.value)}
              style={{ width:'100%', padding:'10px 13px', border:`1px solid ${T.n200}`, borderRadius:2, fontSize:13, fontFamily:'sans-serif', color:T.n800, background:T.white }}>
          ...
          {form.cover_image_url && (
            <div style={{ fontSize:10, color:T.green, fontFamily:'sans-serif', background:T.greenBg, padding:'7px 10px', borderRadius:3, border:`1px solid ${T.greenBorder}` }}>
              ✓ Template uploaded — will appear on the invitation
            </div>
          )}
        </div>
        <div style={{ display:'flex', gap:10, marginTop:22, justifyContent:'flex-end', flexWrap:'wrap' }}>
          <button onClick={onClose} style={{ padding:'10px 18px', background:'none', border:`1px solid ${T.n200}`, borderRadius:2, cursor:'pointer', fontSize:11, letterSpacing:'0.1em', fontFamily:'sans-serif', color:T.n600 }}>
            CANCEL
          </button>
...
function AddGuestModal({ eventId, onDone, onClose }: { eventId: string; onDone: () => void; onClose: () => void }) {
  ...
  const importCSV = async () => {
    ...
    const rows = lines.slice(start).map(line => {
      const [first_name='', last_name='', email='', phone=''] = line.split(',').map(s => s.trim().replace(/^"|"$/g,''))
      return { event_id:eventId, first_name, last_name, email:email||null, phone:phone||null, guest_token:uuidv4(), qr_token:uuidv4(), status:'invited' as const }
    }).filter(r => r.first_name && r.last_name)
    const { error } = await supabase.from('guests').insert(rows)
    setSaving(false)
    if (error) { setMsg('Error: ' + error.message); return }
    setMsg(`${rows.length} guests imported!`)
    setTimeout(() => { onDone(); onClose() }, 1000)
  }

  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ background:T.white, borderRadius:4, padding:'28px 24px', maxWidth:480, width:'100%', border:`1px solid ${T.n200}` }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          <div style={{ fontSize:17, fontFamily:"'Georgia',serif", color:T.n800, fontWeight:700 }}>Add Guests</div>
          <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', fontSize:18, color:T.n400 }}>✕</button>
        </div>
        {/* Tabs */}
        <div style={{ display:'flex', gap:0, borderBottom:`1px solid ${T.n200}`, marginBottom:20 }}>
        ...
        {tab === 'csv' && (
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            <div style={{ fontSize:11, color:T.n600, fontFamily:'sans-serif', lineHeight:1.7, background:T.n100, padding:'10px 12px', borderRadius:3 }}>
              Format: <code>first_name,last_name,email,phone</code><br/>
              One guest per line. Header row optional.
            </div>
            <textarea value={csv} onChange={e => setCsv(e.target.value)}
              placeholder={"Marco,Conti,m.conti@example.com,+39 02 123\nElena,Vasquez,e.vasquez@example.com,"}
              rows={6}
        ...
        {msg && (
          <div style={{ marginTop:12, padding:'8px 12px', background:msg.startsWith('Error')?T.redBg:T.greenBg, border:`1px solid ${msg.startsWith('Error')?T.redBorder:T.greenBorder}`, borderRadius:3, fontSize:12, color:msg.startsWith('Error')?T.red:T.green, fontFamily:'sans-serif' }}>
            {msg}
          </div>
        )}
      </div>
...
export default function AdminPage() {
  ...
  const [guestCounts, setGuestCounts] = useState<Record<string,{total:number,reg:number,in:number}>>({})
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      setEvents(data ?? [])
      // Load guest counts for all events
      const { data: guests } = await supabase.from('guests').select('event_id,status')
      if (guests) {
        const counts: Record<string,{total:number,reg:number,in:number}> = {}
        for (const g of guests) {
          if (!counts[g.event_id]) counts[g.event_id] = {total:0,reg:0,in:0}
          counts[g.event_id].total++
          if (g.status !== 'invited') counts[g.event_id].reg++
          if (g.status === 'checked-in') counts[g.event_id].in++
        }
  ...
  const saveEvent = async (form: EventFormData) => {
    ...
    try {
      ...
      if (editEvt) {
        // Upload new image if it changed and is a base64 string
        if (imageUrl && imageUrl.startsWith('data:')) {
          imageUrl = await uploadImage(imageUrl, editEvt.id)
        }
        ...
        // New event — create first to get the ID, then upload image
        const { data, error } = await supabase.from('events').insert({
          name: form.name, subtitle: form.subtitle||null, date: form.date, time: form.time,
          location: form.location, address: form.address||null, description: form.description||null,
          status: form.status, cover_image_url: null,
        }).select().single()
        if (error) throw error
        // Upload image with real event ID
        if (imageUrl && imageUrl.startsWith('data:')) {
          imageUrl = await uploadImage(imageUrl, data.id)
          if (imageUrl) {
            await supabase.from('events').update({ cover_image_url: imageUrl }).eq('id', data.id)
          }
        }
      }
  ...
  // ── Duplicate event ──
  const duplicateEvent = async (evt: XinaoEvent) => {
    const { error } = await supabase.from('events').insert({
      name: `${evt.name} (Copy)`, subtitle: evt.subtitle, date: evt.date, time: evt.time,
      location: evt.location, address: evt.address, description: evt.description,
      status: 'upcoming', cover_image_url: evt.cover_image_url,
    })
    if (error) { setError(error.message); return }
    await loadEvents()
  }
    ...
    <div style={{ minHeight:'100vh', background:T.black, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ width:'100%', maxWidth:340, textAlign:'center' }}>
        ...
        <div style={{ background:'#111', border:`1px solid ${pwdErr?'#9F0E10':'#1A1A1A'}`, borderRadius:4, padding:28 }}>
          ...
          <button onClick={login} style={{ width:'100%', padding:12, background:T.gold, color:T.black, border:'none', borderRadius:2, cursor:'pointer', fontSize:11, fontWeight:700, letterSpacing:'0.18em', fontFamily:'sans-serif', marginTop:4 }}>
            SIGN IN
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ fontFamily:"'Georgia',serif", background:T.n100, minHeight:'100vh' }}>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'24px 16px' }}>
        ...
        {error && (
          <div style={{ marginBottom:16, padding:'10px 14px', background:T.redBg, border:`1px solid ${T.redBorder}`, borderRadius:3, fontSize:12, color:T.red, fontFamily:'sans-serif' }}>
            ⚠ {error}
          </div>
        )}
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:24, gap:12, flexWrap:'wrap' }}>
          <div>
            ...
            <div style={{ fontSize:11, color:T.n400, fontFamily:'sans-serif', marginTop:2 }}>
        ...
        {/* Loading state */}
        {loading && (
          <div style={{ textAlign:'center', padding:'48px', color:T.n400, fontFamily:'sans-serif', fontSize:13 }}>
            Loading events…
          </div>
        )}
        ...
        {!loading && events.length === 0 && (
          <div style={{ textAlign:'center', padding:'48px 20px', color:T.n400, fontFamily:'sans-serif', fontSize:13, border:`1px dashed ${T.n200}`, borderRadius:4 }}>
            No events yet. Create your first event with "+ NEW EVENT".
          </div>
        )}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:12 }}>
          {sorted.map(evt => {
            const c = guestCounts[evt.id] ?? { total:0, reg:0, in:0 }
            ...
            return (
              <div key={evt.id} style={{ background:T.white, border:`1px solid ${T.n200}`, borderRadius:4, overflow:'hidden', position:'relative' }}>
                {evt.cover_image_url && (
                  <img src={evt.cover_image_url} alt="" style={{ width:'100%', height:80, objectFit:'cover', display:'block' }}/>
                )}
                ...
                <div style={{ padding:'16px 14px', cursor:'pointer' }} onClick={() => router.push(`/admin/events/${evt.id}`)}>
                  ...
                  <div style={{ display:'flex', gap:14 }}>
                    {[['INVITED',c.total],['REG.',c.reg],['IN',c.in]].map(([l,v]) => (
                      <div key={String(l)}>
                        <div style={{ fontSize:7, letterSpacing:'0.18em', color:T.n400, fontFamily:'sans-serif' }}>{l}</div>
                        <div style={{ fontSize:18, fontFamily:"'Georgia',serif", color:T.n800, fontWeight:700 }}>{v}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ position:'absolute', top:evt.cover_image_url?88:8, right:8 }}>
                  <button onClick={e => { e.stopPropagation(); setMenuOpen(menuOpen===evt.id?null:evt.id) }}
                    style={{ width:26, height:26, background:T.white, border:`1px solid ${T.n200}`, borderRadius:2, cursor:'pointer', fontSize:13, color:T.n400, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 1px 4px rgba(0,0,0,0.07)' }}>
                    ⋯
                  </button>
                  {menuOpen === evt.id && (
                    <div style={{ position:'absolute', top:29, right:0, background:T.white, border:`1px solid ${T.n200}`, borderRadius:4, boxShadow:'0 4px 20px rgba(0,0,0,0.1)', zIndex:100, minWidth:160, overflow:'hidden' }}>
                      {[
                        { l:'✎  Edit',        a: () => { setMenuOpen(null); setEditEvt(evt); setFormOpen(true) } },
                        { l:'+ Add Guests',   a: () => { setMenuOpen(null); setAddGuestEvt(evt.id) } },
                        { l:'⧉  Duplicate',   a: () => { setMenuOpen(null); duplicateEvent(evt) } },
                        { l:'✕  Delete',      a: () => { setMenuOpen(null); setDelEvt(evt) }, danger: true },
                      ].map(item => (
                        <button key={item.l} onClick={item.a}
                          style={{ display:'block', width:'100%', padding:'9px 13px', background:'none', border:'none', borderBottom:`1px solid ${T.n100}`, cursor:'pointer', fontSize:11, fontFamily:'sans-serif', color:(item as any).danger?T.red:T.n800, textAlign:'left' }}>
                          {item.l}
                        </button>
                      ))}
                    </div>
      ...
      {delEvt && <Modal title="Delete Event" message={`Delete "${delEvt.name}"? This removes all associated guests and cannot be undone.`} onConfirm={confirmDel} onCancel={() => setDelEvt(null)} danger />}
