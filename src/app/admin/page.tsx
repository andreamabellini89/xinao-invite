...
function EventFormModal({ initial, onSave, onClose }: {
  initial?: XinaoEvent|null; onSave:(f:EventFormData)=>Promise<void>; onClose:()=>void
}) {
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.55)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:16,overflowY:'auto'}}>
      <div style={{background:T.white,borderRadius:4,padding:'28px 24px',maxWidth:520,width:'100%',border:`1px solid ${T.n200}`,maxHeight:'90vh',overflowY:'auto'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          ...
          {form.cover_image_url&&<div style={{fontSize:10,color:T.green,fontFamily:'sans-serif',background:T.greenBg,padding:'7px 10px',borderRadius:3,border:`1px solid ${T.greenBorder}`}}>✓ Template uploaded</div>}
        </div>
        <div style={{display:'flex',gap:10,marginTop:22,justifyContent:'flex-end',flexWrap:'wrap'}}>
          ...
          <button onClick={handleSave} disabled={!valid||saving} style={{padding:'10px 20px',background:valid&&!saving?T.n800:T.n300,color:valid&&!saving?T.white:T.n400,border:'none',borderRadius:2,cursor:valid&&!saving?'pointer':'not-allowed',fontSize:11,letterSpacing:'0.15em',fontFamily:'sans-serif',fontWeight:700}}>
...
export default function AdminPage() {
  ...
  // ── Persistent auth — stored in sessionStorage ──
  ...
  const saveEvent = async (form: EventFormData) => {
    ...
    try {
      ...
      if (editEvt) {
        if (imageUrl?.startsWith('data:')) imageUrl = await uploadImage(imageUrl, editEvt.id)
        const { error } = await supabase.from('events').update({...form,cover_image_url:imageUrl,updated_at:new Date().toISOString()}).eq('id',editEvt.id)
        if (error) throw error
      } else {
        ...
        if (imageUrl?.startsWith('data:')) {
          ...
          if (imageUrl) await supabase.from('events').update({cover_image_url:imageUrl}).eq('id',data.id)
        }
      }
    ...
    <div style={{fontFamily:"'Georgia',serif",background:T.n100,minHeight:'100vh'}}>
      ...
      <div style={{background:T.black,borderBottom:'1px solid #1A1A1A',padding:'0 16px',position:'sticky',top:0,zIndex:200}}>
        <div style={{maxWidth:1100,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',height:46}}>
          ...
          <button onClick={logout} style={{padding:'4px 11px',background:'none',border:'1px solid #2A2520',borderRadius:2,cursor:'pointer',fontSize:9,letterSpacing:'0.14em',fontFamily:'sans-serif',color:T.n600}}>
            LOGOUT
          </button>
        </div>
      </div>
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'24px 16px'}}>
        ...
        {!loading&&events.length===0&&<div style={{textAlign:'center',padding:'40px 20px',color:T.n400,fontFamily:'sans-serif',fontSize:13,border:`1px dashed ${T.n200}`,borderRadius:4}}>No events yet. Create your first event.</div>}
          ...
          {sorted.map(evt=>{
            ...
            return (
              <div key={evt.id} style={{background:T.white,border:`1px solid ${T.n200}`,borderRadius:4,overflow:'hidden',position:'relative'}}>
                ...
                <div style={{padding:'14px 14px',cursor:'pointer'}} onClick={()=>router.push(`/admin/events/${evt.id}`)}>
      ...
      {delEvt&&<Modal title="Delete Event" message={`Delete "${delEvt.name}"? All guests will be removed. Cannot be undone.`} onConfirm={confirmDel} onCancel={()=>setDelEvt(null)} danger/>}
