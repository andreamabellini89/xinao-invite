...
export default function EventDetailPage() {
  ...
  const checkIn = async (guestId: string) => {
    ...
    setGuests(p => p.map(g => g.id===guestId ? {...g, status:'checked-in', checked_in_at:now} : g))
  ...
  const checkQR = async (qr: string) => {
    ...
    await checkIn(g.id)
    setScanResult({ type:'success', guest:{...g, status:'checked-in', checked_in_at:new Date().toISOString()} })
  }
  const simulateScan = () => {
    setScanning(true)
    setTimeout(() => {
  ...
  // ── Loading / Error ──
  if (loading) return (
    <div style={{ minHeight:'100vh', background:T.n100, display:'flex', alignItems:'center', justifyContent:'center', color:T.n400, fontFamily:'sans-serif', fontSize:13 }}>
      Loading event…
    </div>
  )
    ...
    <div style={{ fontFamily:"'Georgia',serif", background:T.n100, minHeight:'100vh' }}>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'24px 16px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20, gap:10, flexWrap:'wrap' }}>
          ...
          <button onClick={() => router.push('/admin')} style={{ padding:'8px 14px', background:'none', border:`1px solid ${T.n200}`, borderRadius:2, cursor:'pointer', fontSize:9, letterSpacing:'0.15em', fontFamily:'sans-serif', color:T.n600, flexShrink:0 }}>
            ✎ EDIT
          </button>
        </div>
          ...
          <div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              ...
              {guests.filter(g=>g.registered_at).length === 0 && (
                <div style={{ padding:'22px', textAlign:'center', color:T.n400, fontSize:12, fontFamily:'sans-serif', background:T.white, border:`1px solid ${T.n200}`, borderRadius:4 }}>
                        ...
                        <div style={{ display:'flex', flexDirection:'column', gap:11, minWidth:200, flex:1 }}>
                          ...
                          {g.status === 'registered' && (
                            <button onClick={() => { checkIn(g.id); setPreview(null) }}
                              style={{ padding:'8px 14px', background:T.n800, color:T.gold, border:'none', borderRadius:2, cursor:'pointer', fontSize:9, letterSpacing:'0.15em', fontFamily:'sans-serif', marginTop:4 }}>
                              MANUAL CHECK-IN
                            </button>
                          )}
                        </div>
          ...
          <div style={{ maxWidth:320, margin:'0 auto', paddingTop:20 }}>
            <div style={{ background:T.black, borderRadius:4, padding:'24px 20px' }}>
              ...
              <div style={{ width:'100%', aspectRatio:'1', background:'#111', border:`1px solid ${scanning?T.gold:'#1A1A1A'}`, borderRadius:4, marginBottom:12, position:'relative', display:'flex', alignItems:'center', justifyContent:'center', transition:'border-color 0.3s' }}>
                ...
                {scanning
                  ? <div style={{textAlign:'center'}}><div style={{fontSize:26,color:T.gold,animation:'spin 1s linear infinite'}}>◎</div><div style={{fontSize:9,color:T.n400,marginTop:5,fontFamily:'sans-serif'}}>SCANNING…</div></div>
