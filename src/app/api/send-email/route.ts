...
export async function POST(req: NextRequest) {
  try {
    ...
    // Usa i testi salvati sull'evento, altrimenti i default da email-config.ts
    const copy   = event.email_copy ?? null
    const invitedToEn = copy?.invited_to_en ?? DEFAULT_COPY.invitedTo.en
    const invitedToZh = copy?.invited_to_zh ?? DEFAULT_COPY.invitedTo.zh
    const confirmEn   = copy?.confirm_en    ?? DEFAULT_COPY.confirmPrompt.en
    const confirmZh   = copy?.confirm_zh    ?? DEFAULT_COPY.confirmPrompt.zh
  ...
  <div style="max-width:520px;margin:40px auto;background:#F8F5EF;border:1px solid #E0D8CC;">
    ...
    <div style="padding:40px 44px;text-align:center;">
      ...
      <div style="font-size:14px;color:#3A3028;line-height:1.8;margin-bottom:20px;">
        ${invitedToEn}<br/>
        <span style="font-size:12px;color:#8A8078;font-family:sans-serif;">${invitedToZh}</span><br/>
      ...
      <div style="font-size:14px;color:#3A3028;line-height:1.8;margin-bottom:32px;">
        ${confirmEn}<br/>
        <span style="font-size:12px;color:#8A8078;font-family:sans-serif;">${confirmZh}</span>
      </div>
